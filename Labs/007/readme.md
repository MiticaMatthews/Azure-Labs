...
```
openssl pkcs12 -in "${USERNAME}Cert.pem" -inkey "${USERNAME}Key.pem" -certfile caCert.pem -export -out "${USERNAME}.p12" -password pass:test123
```
...
3. Unzip the file on your machine. You should see the following contents in the zip file.
   <img width="781" alt="Screenshot 2024-09-12 at 18 04 50" src="https://github.com/user-attachments/assets/8387843d-55a5-4bd3-8b74-567867abd88c">

4. Before we configure the VPN client, we need to add the certificate to our machine's root. To do this, double-click on the **p12** file and you should be prompted for the password. The password is the one set in previous steps, e.g. `test123`. Select **OK**. You should now be able to see the certificate added to the root certificates in keychain.

5. Navigate to **System Settings > Network** on your machine and select **Add VPN Configuration**, and then **IKEv2**.
