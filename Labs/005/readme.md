...
8. Run the following command in your browser terminal to enable IP forwarding:
   `sudo vim /etc/sysctl.conf`
9. Using the text editor of your choice, remove the `#` from the line `net.ipv4.ip_forward=1`, and then press save and exit. Since I used vim, I pressed `i` to inset and edit the file, and then `esc` + `:wq` + `Enter` to save and exit.
10. Close the **Bastion** session.
